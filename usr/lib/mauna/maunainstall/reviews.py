...
REVIEWS_CACHE = os.path.join(GLib.get_user_cache_dir(), "maunainstall", "reviews.json")
        ...
        try:
            ...
            if path is not None:
                try:
                    ...
                    print("MaunaInstall: Cannot open reviews cache: %s" % str(e))
                    cache = {}
                    size = 0
    ...
    def _save_cache(self, cache, size):
        ...
        try:
            ...
            try:
                ...
                print("MaunaInstall: Could not save review cache: %s" % str(e))
        ...
        try:
            ...
            if r.status_code == 200:
                if int(r.headers.get("content-length")) != current_size.value:
                    ...
                    print("MaunaInstall: Downloaded new reviews")
                    success.value = True
                else:
                    print("MaunaInstall: No new reviews")
            else:
                print("MaunaInstall: Could not download updated reviews: %s" % r.reason)
                success.value = False
        except Exception as e:
            print("MaunaInstall: Problem attempting to access reviews url: %s" % str(e))
