...
FALLBACK_PACKAGE_ICON_PATH = "/usr/share/mauna/maunainstall/data/available.png"
VERIFIED_IMAGE_PATH = "/usr/share/mauna/maunainstall/data/verified.svg"
...
import setproctitle
setproctitle.setproctitle("maunainstall")

SCREENSHOT_DIR = os.path.join(GLib.get_user_cache_dir(), "maunainstall", "screenshots")

Gtk.IconTheme.get_default().append_search_path("/usr/share/mauna/maunainstall")
...
ALIASES['mauna-update'] = "Mauna Update"
...
libdir = os.path.join("/usr/lib/mauna/maunainstall")
...
class ScreenshotDownloader(threading.Thread):
    ...
    def save_to_file(self, url, source_url, path):
        ...
        info.set_attribute_string("metadata::maunainstall-screenshot-source-url", source_url)
        try:
...
class BannerTile(Gtk.FlowBoxChild):
    def __init__(self, pkginfo, installer, name, is_flatpak, show_verified, app_json, on_clicked_action):
        ...
        image_uri = (f"/usr/share/mauna/maunainstall/featured/{name}.svg")
...
class PackageRow(Gtk.ListBoxRow):
    def __init__(self, pkginfo, icon, summary, installer, from_search=False, review_info=None):
        ...
        glade_file = "/usr/share/mauna/maunainstall/package-row.glade"
    ...
    def refresh_state(self):
        ...
        if self.installed:
            self.installed_mark.set_from_icon_name("maunainstall-installed", Gtk.IconSize.LARGE_TOOLBAR)
        else:
...
class VerticalPackageTile(Gtk.FlowBoxChild):
    def __init__(self, pkginfo, icon, installer, show_package_type=False, review_info=None):
        ...
        glade_file = "/usr/share/mauna/maunainstall/vertical-tile.glade"
        self.builder = Gtk.Builder()
        self.builder.add_from_file(glade_file)
    ...
    def refresh_state(self):
        ...
        if self.installed:
            self.installed_mark.set_from_icon_name("maunainstall-installed", Gtk.IconSize.MENU)
        else:
            self.installed_mark.clear()
...
class SubcategoryFlowboxChild(Gtk.FlowBoxChild):
    def __init__(self, category, is_all=False, active=False):
        ...
        if is_all:
            cat_name = _("All")
            cat_icon = "maunainstall-all-symbolic"
...
class Application(Gtk.Application):
    ...
    def __init__(self):
        ...
        print("MaunaInstall: Detected system architecture: '%s'" % self.arch)
    ...
    def do_command_line(self, command_line, data=None):
        ...
        if num > 1 and args[1] == "list":
            ...
            print("MaunaInstall: file not found", args[2])
            sys.exit(1)
        elif num > 1:
            print("MaunaInstall: Unknown arguments", args[1:])
            sys.exit(1)
    ...
    def get_low_res_screen(self):
        ...
        if height < 800:
            print("MaunaInstall: low resolution detected (%dpx height), limiting window height." % (height))
            return True
    ...
    def create_window(self, starting_page):
        if self.main_window is not None:
            print("MaunaInstall: create_window called, but we already had one!")
            return
        ...
        self.main_window.set_icon_name("maunainstall")
    ...
    def load_banner(self):
        ...
        json_array = json.load(open("/usr/share/mauna/maunainstall/featured/featured.json", "r"))
    ...
    def load_featured(self):
        ...
        apps = []
        featured_list = self.file_to_array("/usr/share/mauna/maunainstall/categories/picks.list")
    ...
    def get_screenshot_source_from_metadata(self, screenshot):
        ...
        try:
            info = file.query_info("metadata::maunainstall-screenshot-source-url", Gio.FileQueryInfoFlags.NONE, None)
            url = info.get_attribute_string("metadata::maunainstall-screenshot-source-url")
            if url is not None:
                return url
        except GLib.Error as e:
    ...
    def open_about(self, widget):
        ...
        dlg.set_program_name("maunainstall")
        ...
        dlg.set_icon_name("maunainstall")
        dlg.set_logo_icon_name("maunainstall")
    ...
    def add_categories(self):
        ...
        self.flatpak_category = Category("Flatpak", None, self.categories, "maunainstall-package-flatpak-symbolic")

        # ALL
        self.all_category = Category(_("All Applications"), None, self.categories, "maunainstall-all-symbolic")
        with os.scandir("/usr/share/mauna/maunainstall/categories/") as it:
            for entry in it:
        ...
        # INTERNET
        category = Category(_("Internet"), None, self.categories, "maunainstall-web-symbolic")

        subcat = Category(_("Web"), category, self.categories, "maunainstall-web-symbolic")
        self.sections["web"] = subcat
        self.sections["net"] = subcat
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/internet-web.list")

        subcat = Category(_("Email"), category, self.categories, "maunainstall-email-symbolic")
        self.sections["mail"] = subcat
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/internet-email.list")

        subcat = Category(_("Chat"), category, self.categories, "maunainstall-chat-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/internet-chat.list")

        subcat = Category(_("File sharing"), category, self.categories, "maunainstall-share-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/internet-filesharing.list")

        self.root_categories[category.name] = category

        # SOUND AND VIDEO
        category = Category(_("Sound and video"), None, self.categories, "maunainstall-music-symbolic")
        category.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/sound-video.list")
        subcat = Category(_("Sound"), category, self.categories, "maunainstall-music-symbolic")
        self.sections["sound"] = subcat
        subcat = Category(_("Video"), category, self.categories, "maunainstall-video-symbolic")
        self.sections["video"] = subcat
        self.root_categories[category.name] = category

        # GRAPHICS
        category = Category(_("Graphics"), None, self.categories, "maunainstall-drawing-symbolic")
        self.sections["graphics"] = category
        category.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics.list")

        subcat = Category(_("3D"), category, self.categories, "maunainstall-3d-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics-3d.list")
        subcat = Category(_("Drawing"), category, self.categories, "maunainstall-drawing-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics-drawing.list")
        subcat = Category(_("Photography"), category, self.categories, "maunainstall-photo-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics-photography.list")
        subcat = Category(_("Publishing"), category, self.categories, "maunainstall-publishing-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics-publishing.list")
        subcat = Category(_("Scanning"), category, self.categories, "maunainstall-scanning-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics-scanning.list")
        subcat = Category(_("Viewers"), category, self.categories, "maunainstall-viewers-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/graphics-viewers.list")
        self.root_categories[category.name] = category

        # OFFICE
        category = Category(_("Office"), None, self.categories, "maunainstall-office-symbolic")
        self.sections["office"] = category
        self.sections["editors"] = category
        self.root_categories[category.name] = category

        # GAMES
        category = Category(_("Games"), None, self.categories, "maunainstall-games-symbolic")
        self.sections["games"] = category
        category.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games.list")

        subcat = Category(_("Board games"), category, self.categories, "maunainstall-board-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games-board.list")
        subcat = Category(_("First-person"), category, self.categories, "maunainstall-fps-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games-fps.list")
        subcat = Category(_("Real-time strategy"), category, self.categories, "maunainstall-rts-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games-rts.list")
        subcat = Category(_("Turn-based strategy"), category, self.categories, "maunainstall-tbs-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games-tbs.list")
        subcat = Category(_("Emulators"), category, self.categories, "maunainstall-emulator-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games-emulators.list")
        subcat = Category(_("Simulation and racing"), category, self.categories, "maunainstall-sim-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/games-simulations.list")
        self.root_categories[category.name] = category

        # ACCESSORIES
        category = Category(_("Accessories"), None, self.categories, "maunainstall-accessories-symbolic")
        self.sections["accessories"] = category
        self.sections["utils"] = category
        self.root_categories[category.name] = category

        # SYSTEM TOOLS
        category = Category(_("System tools"), None, self.categories, "maunainstall-system-symbolic")
        self.sections["system"] = category
        self.sections["admin"] = category
        category.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/system-tools.list")
        self.root_categories[category.name] = category

        # FONTS
        category = Category(_("Fonts"), None, self.categories, "maunainstall-fonts-symbolic")
        self.sections["fonts"] = category
        category.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/fonts.list")
        self.root_categories[category.name] = category

        # EDUCATION
        category = Category(_("Science and Education"), None, self.categories, "maunainstall-science-symbolic")
        subcat = Category(_("Science"), category, self.categories, "maunainstall-science-symbolic")
        self.sections["science"] = subcat
        subcat = Category(_("Maths"), category, self.categories, "maunainstall-maths-symbolic")
        self.sections["math"] = subcat
        subcat = Category(_("Education"), category, self.categories, "maunainstall-education-symbolic")
        self.sections["education"] = subcat
        subcat = Category(_("Electronics"), category, self.categories, "maunainstall-electronic-symbolic")
        self.sections["electronics"] = subcat
        category.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/education.list")
        self.root_categories[category.name] = category

        # PROGRAMMING
        category = Category(_("Programming"), None, self.categories, "maunainstall-programming-symbolic")
        self.sections["devel"] = category
        subcat = Category(_("Java"), category, self.categories, "maunainstall-java-symbolic")
        self.sections["java"] = subcat
        subcat = Category(_("PHP"), category, self.categories, "maunainstall-php-symbolic")
        self.sections["php"] = subcat
        subcat = Category(_("Python"), category, self.categories, "maunainstall-python-symbolic")
        self.sections["python"] = subcat
        subcat = Category(_("Essentials"), category, self.categories, "xapp-favorites-app-symbolic")
        subcat.matchingPackages = self.file_to_array("/usr/share/mauna/maunainstall/categories/development-essentials.list")
        self.root_categories[category.name] = category
    ...
    def show_package(self, pkginfo, previous_page):
        ...
        if pkginfo.pkg_hash.startswith("f") or self.get_flatpak_for_deb(pkginfo) is not None:
            ...
            for remote in self.installer.list_flatpak_remotes():
                ...
                if row_pkginfo:
                    row = [i, _("Flatpak (%s)") % remote.title, remote.summary, "maunainstall-package-flatpak-symbolic", row_pkginfo]
                    iter = self.package_type_store.append(row)
                    if pkginfo == row_pkginfo:
                        to_use_iter = iter
                        tooltip = row[PACKAGE_TYPE_COMBO_SUMMARY]
